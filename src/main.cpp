...
void createSquareSpriteBuffers()
{
	...
	std::array<float, vertices_count> square_vertices {
	...
	std::array<unsigned, indices_count> square_indices {
...
void keyCallback(GLFWwindow *window, int key, int, int action, int)
{
	...
	if (key >= 0 && key < static_cast<int>(settings->input.size()))
	{
		if (action == GLFW_PRESS)
		{
			settings->input.at(key) = true;
		}
		else if (action == GLFW_RELEASE)
		{
			settings->input.at(key)			  = false;
			settings->input_processed.at(key) = false;
		}
	}
}
...
void mouseCallback(GLFWwindow *window, int button, int action, int)
{
	...
	if (action == GLFW_PRESS)
	{
		settings->input.at(button) = true;
	}
	else if (action == GLFW_RELEASE)
	{
		settings->input.at(button)			 = false;
		settings->input_processed.at(button) = false;
	}
}
...
void drawSprite(const Settings &settings, const char *sprite, float x, float y)
{
	...
	glUseProgram(settings.shaders.at(sprite));

	glUniformMatrix4fv(
		glGetUniformLocation(settings.shaders.at(sprite), "model"),
		1,
		GL_FALSE,
		glm::value_ptr(model));
